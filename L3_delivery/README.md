...
- **深度**：唯一稳健短板是**中文 QA**（非抽取题 43.6% vs 官方 54.8%，比较题 2.0% vs 9.4%），经双判官+客观+人工四重验证。英文深度与官方持平。

**rewrite 线**（本地有原文可评全维度；官方无原文只评部分维度）：

| 维度 | 中文官方 | 中文本地 | 英文官方 | 英文本地 |
|---|---|---|---|---|
| 忠实(仅本地) | — | **4.92** | — | **4.83** |
| 自然度 | 3.57 | **4.92** | 4.14 | **4.88** |
| 深度 | 2.86 | **4.83** | 3.78 | **4.75** |

- rewrite 本地质量**全面达到并超过官方**：忠实近满分，自然度/深度高于官方（去噪+理顺是本地强项，官方成品反带噪声）。人工审核四风格全部“忠实性通过”。

> 口径提示：QA closeness 用 DMX、绝对质量 4 维用 35b；“自然度”因两判官相关 r≈0 不可信，QA 侧未采信（详见报告）。
