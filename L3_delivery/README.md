...
- **底线质量**（忠实/可答/覆盖）本地不低于官方；中文本地忠实明显更高（4.98 vs 3.93）——推测因官方种子含广告/噪声原文、本地有 L0 数据卫生护栏过滤（**数据卫生差异之推断，未做 pipeline 控制实验证实**）。注：本表官方(前100切片)与本地(aliyun_n100)为**分布对比、非严格同源**；同源对齐版(n=98)官方忠实=4.0、本地=4.98，方向一致。
- **深度**：官方本地都有原文、判官口径对等；双判官(35b/qmax)+客观统计三方一致确认——唯一稳健短板是**中文 QA**（客观非抽取题 43.6% vs 官方 54.8%，比较题 2.0% vs 9.4%），另经人工审核验证。英文深度与官方持平（英文本地采用封板 v1-hardened 同源产出 baseline，35b=2.02/qmax=2.33，客观非抽取题占比50.0%；另有一批非同源产出深度35b=1.64，该批未单独跑客观统计，但同源批客观≈50%与官方持平，两判官也显示接近，结论稳健）。
...
**吞吐甜点（5090 实测，见吞吐报告）**：QA 并发 16（中文 c16 实测 **85.7 篇/min**，测于 v3.x 强制题型族 prompt、200 篇；英文 62 篇/min）；rewrite 并发 8（zh 13.6 篇/min、en 10.33 篇/min）。判官必须 2 并发（高并发下 8192 显存竞争会截断 JSON）；rewrite 判官 anchor 默认 4（16 个会撑爆 8192 输入）。
...
|------|-----|---------|------|---------|--------|--------|
| 中文 | QA | v3.x强制题型族 | 16 | **85.7 篇/分**（200篇实测；v3.35 同族prompt、5090复验成功率100%/0泄漏，未单独测v3.35墙钟） | 1.0 | 0 |
