"""Extract original seed text from official en L3 QA content.

English counterpart of extract_seed_from_l3.py. The zh version keys on the
"问题：" / "答案：" markers; the official en QA content uses "Question:" / "Answer:"
(confirmed by inspecting data/ultrafineweb_en_l3/qa shards). This module mirrors
the zh API (split_content_with_reason / split_content) so the eval harness can use
either by language. New file — the zh splitter is left untouched.
"""
...
def split_content_with_reason(content: str) -> SplitResult | SplitFailure:
    """Split official en QA content, retaining exact characters on both sides."""
...
def split_content(content: str) -> tuple[str, str] | None:
    """Return (seed_text, official_qa_pairs) if content passes en QA split checks."""
